...
def kalman_filter(x_hat, P, control, measurement, R, Q):

    # State transition matrix
    F = np.array([
        [1, 0, -control[0] * np.sin(x_hat[2])],
        [0, 1, control[0] * np.cos(x_hat[2])],
        [0, 0, 1]
    ])
...
def getEclipse(covariance_history, estimated_trajectory):
    # Plot Uncertainty Ellipsoids
    print(covariance_history)
    cov_matrix = covariance_history[:2, :2]  # Extract covariance for x and y
    mean = estimated_trajectory[:2]  # Extract mean for x and y
    ...
    ellipse = patches.Ellipse(xy=mean, width=width, height=height, angle=angle, alpha=0.3, color='red')
    return ellipse
...
def update(frame, P, phistory, controls, car, car2, visited,estimates, trace, distances,landmarks, ptrace, N):
    ...
    process_noise = np.diag([0.01, 0.01, 0.001])
    
    estimate, P = kalman_filter([0,0,0], P, controls[frame], (car.q[0], car.q[1]), measurement_noise, process_noise)
    estimates.append((estimate[0], estimate[1]))
    phistory.append(P)
